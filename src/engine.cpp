...
Mesh Mesh::load(string path) {
  ...
  const aiScene *scene = importer.ReadFile(
      path, aiProcess_CalcTangentSpace | aiProcess_GenSmoothNormals |
  ...
  process_mesh = [&](aiMesh *mesh) {
    for (u32 i = 0; i < mesh->mNumVertices; i++) {
      vertices.push_back(Vec3f(mesh->mVertices[i].x, mesh->mVertices[i].y,
                               mesh->mVertices[i].z));
      ...
      if (mesh->HasTangentsAndBitangents()) {
        tangents.push_back(Vec3f(mesh->mTangents[i].x, mesh->mTangents[i].y,
                                 mesh->mTangents[i].z));

        bitangents.push_back(Vec3f(mesh->mBitangents[i].x,
      ...
      if (mesh->mTextureCoords[0]) {
        uvs.push_back(
            Vec2f(mesh->mTextureCoords[0][i].x, mesh->mTextureCoords[0][i].y));
      } else {
  ...
  return Mesh::make(vertices, uvs, normals, tangents, bitangents, indices);
}

Mesh Mesh::make(ArrayList<Vec3f> vertices, ArrayList<Vec2f> uvs,
                ArrayList<Vec3f> normals, ArrayList<Vec3f> tangents,
                ArrayList<Vec3f> bitangents, ArrayList<u32> indices) {
  Mesh result;
  ...
  glBufferSubData(GL_ARRAY_BUFFER, i,
                  sizeof(result.vertices[0]) * result.vertices.size(),
  ...
  glBufferSubData(GL_ARRAY_BUFFER, i, sizeof(result.uvs[0]) * result.uvs.size(),
                  result.uvs.data());
  glEnableVertexAttribArray(1);
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(result.uvs[0]),
                        (void *)i);
  i += sizeof(result.uvs[0]) * result.uvs.size();

  glBufferSubData(GL_ARRAY_BUFFER, i,
                  sizeof(result.normals[0]) * result.normals.size(),
                  result.normals.data());
  glEnableVertexAttribArray(2);
  glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(result.normals[0]),
                        (void *)i);
  i += sizeof(result.normals[0]) * result.normals.size();

  glBufferSubData(GL_ARRAY_BUFFER, i,
                  sizeof(result.tangents[0]) * result.tangents.size(),
                  result.tangents.data());
  glEnableVertexAttribArray(3);
  glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(result.tangents[0]),
                        (void *)i);
  i += sizeof(result.tangents[0]) * result.tangents.size();

  glBufferSubData(GL_ARRAY_BUFFER, i,
                  sizeof(result.bitangents[0]) * result.bitangents.size(),
                  result.bitangents.data());
  glEnableVertexAttribArray(4);
  glVertexAttribPointer(4, 3, GL_FLOAT, GL_FALSE, sizeof(result.bitangents[0]),
                        (void *)i);
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               sizeof(result.indices[0]) * result.indices.size(),
...
Sprite Sprite::make(string path) {
  ...
  for (auto &[key, value] : data.items()) {
    ...
    for (auto &elem : value["frames"]) {
      metadata.frames.push_back(
          Frame{elem["x"], elem["y"], elem["w"], elem["h"]});
    }
...
Mat4 FPSCamera::GetPerspective() {
  return glm::perspective(glm::radians(fov), (f32)game_width / (f32)game_height,
                          near, far);
}

Vec3f FPSCamera::GetForward() { return forward; }

Vec3f FPSCamera::GetRight() { return right; }

Vec3f FPSCamera::GetUp() { return up; }
...
void FPSCamera::update() {
  ...
  forward =
      glm::normalize(glm::inverse(rotation) * Vec4f(engine::WorldForward, 1));
...
Mat4 Camera2D::GetPerspective() {
  return glm::ortho(0.0f, (f32)game_width, 0.0f, (f32)game_height, 0.0f,
                    1000.0f);
}
...
Framebuffer Framebuffer::make(bool depth_test = false) {
  ...
  glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, game_width,
                        game_height);
  glBindRenderbuffer(GL_RENDERBUFFER, 0);
  glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT,
                            GL_RENDERBUFFER, result.rbo);
...
Vec2f mouse_delta() { return mousedelta; }

Vec2f mouse_pos() { return mousepos; }
...
void lock_mouse() { mouselock = true; }

void unlock_mouse() { mouselock = false; }

SDL_GLContext get_gpu_context() { return context; }

SDL_Window *get_window() { return window; }

string get_GLSLVersion() { return glslVersion; }

Vec2f get_game_size() { return Vec2f(game_width, game_height); }

void set_framebuffer(Framebuffer &frame) {}

void set_clear_color(f32 x, f32 y, f32 z) { clearcolor = Vec3f(x, y, z); }
...
int main(int argc, char **argv) {
  ...
  const char *exe_path_ptr = SDL_GetBasePath();
  exe_path = string(exe_path_ptr);
  delete exe_path_ptr;

  window = SDL_CreateWindow(title.c_str(), sdl_width, sdl_height,
  ...
  if (!context) {
    std::cout << "SDL_Error: " << "Failed to initialize OpenGL context"
              << std::endl;
    return 1;
  } else
  ...
  prevkeyboard = new bool[keyboardsize];
  ...
  ImGui_ImplSDL3_InitForOpenGL(engine::get_window(), engine::get_gpu_context());
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDL_EVENT_MOUSE_MOTION:
        ...
        mousepos = Vec2f(event.motion.x, event.motion.y);

        mousedelta = Vec2f(event.motion.xrel, event.motion.yrel);
    ...
    if (window_focused) {
      if (!SDL_SetWindowRelativeMouseMode(window, mouselock)) {
    ...
    ImGui::GetWindowDrawList()->AddImage(
        (void *)(isize)framebuffer.albedo, ImVec2(pos.x, pos.y),
        ImVec2(pos.x + game_width, pos.y + game_height), ImVec2(0, 1),
        ImVec2(1, 0));
    ...
    prevkeyboard = (bool *)memcpy(prevkeyboard, keyboard,
                                  keyboardsize * sizeof(*keyboard));
  }
